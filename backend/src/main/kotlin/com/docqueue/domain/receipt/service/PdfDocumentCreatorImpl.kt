...
class PdfDocumentCreatorImpl(
    ...
    override suspend fun createPdfDocument(receipt: Receipt, receiptId: String): Result<ByteArray> = runCatching {
        withContext(Dispatchers.IO) {
            logger.info("PDF 생성 시작: receipt=$receipt, receiptId=$receiptId")
                ...
                PdfWriter(outputStream).use { pdfWriter ->
                    PdfDocument(pdfWriter).use { pdfDocument ->
                        Document(pdfDocument).use { document ->
                            ...
                            val qrContent = "$receiptId|${receipt.phoneNumber}|${receipt.date.format(DateTimeFormatter.ofPattern("yyyyMMdd_HHmmss"))}"
